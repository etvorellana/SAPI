...
def CornerPespec(srcRGB):
    ...
    cv.circle(srcRGB,(plt[1],plt[0]), 30, (0,0,255), -1)
    cv.circle(srcRGB,(plb[1],plb[0]), 30, (0,255,0), -1)
    cv.circle(srcRGB,(prt[1],prt[0]), 30, (255,0,0), -1)
    cv.circle(srcRGB,(prb[1],prb[0]), 30, (0,255,255), -1)
...
def main(argv):
    ...
    #     outFile = "seg_snorm_" + arguments.src
    srcRGB = loadImage(arguments.src) # Carrega Imagem
    outFile = "seg_snorm_" + arguments.src
    ...
    # Thresholding
    thrGray = threshold(srcRGB)

    print("Thresold")
    end = time.time()
    print(end - start)

    img, segList = segment_01(thrGray, srcRGB)
