...
# * Otimizar as funções cornerPespec() e threshold()
...
def loadImage(fileName):
    ...
    srcGray = cv.imread(cv.samples.findFile(fileName), cv.IMREAD_GRAYSCALE)

    # Carrega a imagem colorida
    srcRGB = cv.imread(cv.samples.findFile(fileName))

    print("loadImage() successfully")
...
def filter(srcGray, opt):
    ...
    if opt == 2:
        # Aplica filtro Median Blur
        srcGray = cv.medianBlur(srcGray, 11)

    elif opt == 3:
        # Aplica filtro Gaussian Blur
        srcGray = cv.GaussianBlur(srcGray, (3, 3), 0)
...
def CornerPespec(srcRGB):
    ...
    dsts = cv.blur(srcRGB, (3, 3))

    # Canny
    dst = cv.Canny(dsts, 50, 100, None, 3, True)
    dst = np.float32(dst)
    ...
    cv.imshow("Esquinas", dst)

    # Dilatação para destacar
    dst = cv.dilate(dst, None)
    N, M = dst.shape

    # -i: a partir daqui n entendi muito bem essa parte da dilatação :/
    ...
    pts1 = np.float32([[plt[1], plt[0]], [prt[1], prt[0]],
                       [plb[1], plb[0]], [prb[1], prb[0]]])
    pts2 = np.float32([[0, 0], [dm, 0], [0, dn], [dm, dn]])

    matrix = cv.getPerspectiveTransform(pts1, pts2)
    result = cv.warpPerspective(dsts, matrix, (dm, dn))
    ...
    print("cornerPespec() successfully")
...
def edgesDetection(src, thr1=50, thr2=100, apSize=3, L2Grad=True):
    ...
    print("edgesDetection() successfully")
    return dst
    ...
    if linesP is not None:
        ...
        if dx > dy:  # se ela for maior na direção x esticamos em x
            for x in range(x2 + 1, M):
                if src[y2, x + 1] > 0:  # procuramos primeiro a direita
                    x2 = x
            ...
            for y in range(y2 + 1, N // 2):  # // é divisão arredondadno apra o piso
                if src[y, x2] > 0:  # procuramos primeiro abaixo
                    y2 = y
                elif src[y, x2 + 1] > 0:  # depois procuramos a esquerda
                    y2 = y
                    x2 = x2 + 1
...
def alignment(src, x1, y1, x2, y2):
    ...
    if dx > dy:
        theta = np.rad2deg(math.atan(dy / dx))  # -i: math oq?
    else:
    ...
    # Rotaciona a imagem com a função warpAffine
    rot = cv.warpAffine(src, rotMat, (M, N))
...
def threshold(src, dst):
    ...
    # Utilizando o pacote skimage é realizada a transformação para o espaço de cor YIQ
    cutYIQ = skimage.color.rgb2yiq(src)

    # Terei duas imagens binárias ao final para poder comparar melhor os resultados
    thrGrey_l1 = np.zeros_like(dst)
    thrGrey_l2 = np.zeros_like(dst)

    # Lista que guardará os pixels que serão usados na seguna limiarização
    limiar_2 = []
    ...
    # Essa parte do código está muito lenta
    #   Busca pelo limiar da imagem
    while (con_L0 != con_L1) and (con_H0 != con_H1) and cont < 100:
        cont += 1
        sum_L = 0
        sum_H = 0
        con_L0 = con_L1
    ...
    #   Após encontrar o limiar binariza a imagem(thrGrey_l1), tudo que for maior que o limiar fica branco e o que for menor preto
    print("limiar = ", Y_med)
    for i in range(M):
        for j in range(N):
            if cutYIQ[i, j, 0] >= Y_med:
                ...
                # Salvo as coordenadas dos pixels que ficaram acima do limiar, para usá-los no segundo ciclo de limiarização
                limiar_2.append((i, j, cutYIQ[i, j, 0]))

    # Primeira img binarizada
    cv.namedWindow("limiar 1", cv.WINDOW_NORMAL)
    cv.imshow("limiar 1", thrGrey_l1)

    ## Começando o segundo ciclo de limiarização ##

    # atribui um valor existente na imagem ao Ymin e Ymax
    Y_2_min = limiar_2[0][2]
    Y_2_max = limiar_2[0][2]

    # Procura o menor e o maior valor de limiar_2[][2]
    for i in range(len(limiar_2)):
        if (Y_2_max < limiar_2[i][2]):
            Y_2_max = limiar_2[i][2]

        elif (Y_2_min > limiar_2[i][2]):
            Y_2_min = limiar_2[i][2]

    # Faz a média entre esses dois valores para termos um ponto de partida
    Y_2_med = (Y_2_max + Y_2_min) / 2.0

    print("Y_2_med inicial: ", Y_2_med)
    ...
    #   Busca pelo limiar da imagem (segunda vez)
    while (con_L0 != con_L1) and (con_H0 != con_H1) and cont < 100:
        cont += 1
        sum_L = 0
        sum_H = 0
        ...
        for i in range(len(limiar_2)):
            if limiar_2[i][2] < Y_2_med:
                sum_L += limiar_2[i][2]
                con_L1 += 1

                Y_2_min = sum_L / con_L1

            else:
                sum_H += limiar_2[i][2]
                con_H1 += 1

                Y_2_max = sum_H / con_H1

        Y_2_med = (Y_2_max + Y_2_min) / 2.0

    #   Após encontrar o limiar, binariza a imagem, tudo que for maior que o limiar branco e o que for menor preto
    print("limiar_2 = ", Y_2_med)

    for i in range(len(limiar_2)):
        if limiar_2[i][2] >= Y_2_med:
            thrGrey_l2[limiar_2[i][0], limiar_2[i][1]] = 255

    # Exibe a segunda imagem do limiar
    cv.namedWindow("limiar 2", cv.WINDOW_NORMAL)
    cv.imshow("limiar 2", thrGrey_l2)

    # cv.waitKey()
    print("threshold() successfully")
    return thrGrey_l2  # Retorna a imagem binarizada
...
def segment_01(thrGrey, normRGB):
    ...
    h, w = thrGrey.shape[:2]
    print("Shape: ", thrGrey.shape)
    print("h: ", h)
    print("w: ", w)

    (x_0, y_0) = (0, 0)
    mask = np.zeros((h + 2, w + 2), np.uint8)  # -i: pq uint8?
    ...
    contours, hierarchy = cv.findContours(
        im_out, cv.RETR_TREE, cv.CHAIN_APPROX_SIMPLE)
    ...
    for c in contours:
        M = cv.moments(c)
        if M["m00"] != 0:  # -i: m00 = área (n sei o resto ainda :/)

            # -i: coordenadas dos centroides cX e cY
            cX = int(M["m10"] / M["m00"])
            cY = int(M["m01"] / M["m00"])
            # informações para desenhar um retângulo ao redor da imagem binária
            x, y, w, h = cv.boundingRect(c)

            # checando as dimenssões do retângulo para saber se o mesmo se encaixa no tamanho de uma solda
            if (abs(w - h) < 15) and ((w * h) < 600) and ((w * h) > 40):
...
def main(argv):
    ...
    # O PATH para windows e Linux tem formatos diferentes
    parser.add_argument('--arquivo', action='store', dest='src', default='E:\\1-Study\\CIC\\Bolsa\\ProcImgs\\bancoImgs\\PCB_01_ilumin_03.jpg',
                        required=False, help='Nome do arquivo de imagem ou "foto" para utilizar a camera')

    parser.add_argument('-filtro', type=int, action='store', dest='filtro', default=1, required=False,
                        help='Tipo se filtro a ser aplicado: 1- Sem Filtro; 2 - Median Blur; 3 - Gaussian Blur ')

    parser.add_argument('-borda', type=int, action='store', dest='borda', default=1, required=False,
...
# Aplica o filtro na imagem, caso tenha sido escohido (sem diferença relevante no final)
    if (arguments.filtro == 2) or (arguments.filtro == 3):
        srcGrey = filter(srcGrey, arguments.filtro)

# Decção de bordas  (melhor resultado com 1)
    if (arguments.borda == 1):
        ...
        cv.line(border, (x1, y1), (x2, y2), (0, 0, 255),
                1, cv.LINE_AA)  # Desenha linha na bordas
    ...
    # Thresholding
    # thrGrey = threshold(normRGB, srcGrey)     # melhor resultado
...
# Analisar segList para tentar entender porque alguns quadrados n ficaram fechados
    # file1 = open("segList.txt", "a")
    # file1.write(str(segList))
    # file1.close()
    # Aparentemente n teve nada a ver com os valores da lista
    # Resposta: era só aumentar a espessura do traço
